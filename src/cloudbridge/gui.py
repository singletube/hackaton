...
def launch_gui(initial_config: AppConfig, *, manager_name: str = "auto") -> int:
    ...
    class CloudBridgeGuiApp:
        def __init__(self) -> None:
            self._root = tk.Tk()
            self._root.title("Настройка CloudBridge")
            ...
            self._auth_title_var = tk.StringVar(value="Авторизация")
            self._auth_help_var = tk.StringVar(value="Здесь появятся подсказки для входа в выбранное облако.")
            ...
            self._status_var = tk.StringVar(value=f"Конфиг: {initial_config.resolved_config_path}")
        ...
        def _build_ui(self) -> None:
            ...
            self._ttk.Label(
                header,
                text="Синхронизация облака и настройка рабочего окружения Linux",
                style="HeroSubtitle.TLabel",
            ).grid(row=1, column=0, sticky="w", pady=(6, 0))

            badge = self._ttk.Label(
                header,
                text="палитра BaseALT",
            ...
            notebook.add(general, text="Основное")
            notebook.add(yandex, text="Яндекс")
        ...
        def _build_general_tab(self, frame) -> None:
            self._add_row(frame, 0, "Провайдер", self._combo(frame, self._provider_var, ("yandex", "nextcloud")))
            ...
            browse_button = self._ttk.Button(sync_root_row, text="Выбрать", command=self._choose_sync_root)
            browse_button.grid(row=0, column=1, padx=(8, 0))
            self._add_row(frame, 1, "Папка синхронизации", sync_root_row)

            self._add_row(frame, 2, "Облачная папка для импорта", self._ttk.Entry(frame, textvariable=self._import_root_var))
            self._add_row(frame, 3, "Схема импорта", self._combo(frame, self._import_layout_var, ("flat", "by-parent", "by-date")))
            self._add_row(frame, 4, "Режим отслеживания", self._combo(frame, self._watcher_backend_var, ("auto", "watchdog", "poll")))
            self._add_row(frame, 5, "Параллельность сканирования", self._spinbox(frame, self._scan_concurrency_var, 1, 64))
            self._add_row(frame, 6, "Параллельность синхронизации", self._spinbox(frame, self._sync_concurrency_var, 1, 64))
            self._add_row(frame, 7, "Файловый менеджер", self._combo(frame, self._manager_var, ("auto", "nautilus", "thunar", "nemo", "caja")))
            self._add_row(frame, 8, "Имя службы", self._ttk.Entry(frame, textvariable=self._service_name_var))

            options = self._ttk.LabelFrame(frame, text="Интеграция с рабочим столом", padding=12)
            options.grid(row=9, column=0, columnspan=2, sticky="ew", pady=(16, 0))
            options.grid_columnconfigure(0, weight=1)
            self._ttk.Checkbutton(
                options,
                text="Установить интеграцию с файловым менеджером",
                variable=self._install_filemanager_var,
            ).grid(row=0, column=0, sticky="w")
            self._ttk.Checkbutton(
                options,
                text="Установить пользовательскую службу systemd",
            ...
            save_button = self._ttk.Button(actions, text="Сохранить настройки", style="Secondary.TButton", command=self._save_config)
            save_button.grid(row=0, column=0, sticky="w")
            setup_button = self._ttk.Button(actions, text="Настроить рабочее окружение", style="Primary.TButton", command=self._run_desktop_setup)
        ...
        def _build_yandex_tab(self, frame) -> None:
            self._add_row(frame, 0, "Токен доступа", self._ttk.Entry(frame, textvariable=self._yandex_token_var, show="*"))
            ...
            login_button = self._ttk.Button(actions, text="Начать вход по коду устройства", style="Primary.TButton", command=self._run_yandex_login)
            login_button.grid(row=0, column=0, sticky="w")
            self._operation_buttons.append(login_button)

            self._ttk.Label(
                frame,
                text="Введите код из правой панели на странице Яндекса. Яндекс не присылает этот код письмом или по SMS.",
                style="Muted.TLabel",
                wraplength=540,
                justify="left",
        ...
        def _build_nextcloud_tab(self, frame) -> None:
            self._add_row(frame, 0, "Адрес сервера", self._ttk.Entry(frame, textvariable=self._nextcloud_url_var))
            self._add_row(frame, 1, "Имя пользователя", self._ttk.Entry(frame, textvariable=self._nextcloud_username_var))
            self._add_row(frame, 2, "Пароль приложения", self._ttk.Entry(frame, textvariable=self._nextcloud_password_var, show="*"))

            actions = self._ttk.Frame(frame)
            actions.grid(row=3, column=0, columnspan=2, sticky="w", pady=(20, 0))
            login_button = self._ttk.Button(actions, text="Начать вход через браузер", style="Primary.TButton", command=self._run_nextcloud_login)
            login_button.grid(row=0, column=0, sticky="w")
            self._operation_buttons.append(login_button)

            self._ttk.Label(
                frame,
                text="Ссылка для входа появится в правой панели и в журнале.",
                style="Muted.TLabel",
                wraplength=540,
                justify="left",
            ).grid(row=4, column=0, columnspan=2, sticky="w", pady=(16, 0))
        ...
        def _build_auth_panel(self, frame) -> None:
            ...
            self._ttk.Label(panel, text="Ссылка для входа", style="FieldLabel.TLabel").grid(row=2, column=0, sticky="w")
            ...
            open_button = self._ttk.Button(url_row, text="Открыть", style="Secondary.TButton", command=self._open_auth_url)
            open_button.grid(row=0, column=1, padx=(8, 0))
            copy_url_button = self._ttk.Button(url_row, text="Копировать ссылку", style="Secondary.TButton", command=self._copy_auth_url)
            copy_url_button.grid(row=0, column=2, padx=(8, 0))

            self._ttk.Label(panel, text="Код устройства", style="FieldLabel.TLabel").grid(row=4, column=0, sticky="w")
            ...
            copy_code_button = self._ttk.Button(code_row, text="Копировать код", style="Primary.TButton", command=self._copy_auth_code)
        ...
        def _build_quick_help_panel(self, frame) -> None:
            ...
            panel.grid_columnconfigure(0, weight=1)
            self._ttk.Label(panel, text="Как начать", style="SectionTitleInverse.TLabel").grid(row=0, column=0, sticky="w")
            self._ttk.Label(
                panel,
                text=(
                    "1. Выберите облако и сохраните настройки.\n"
                    "2. Запустите вход в Яндекс или Nextcloud.\n"
                    "3. Возьмите ссылку и код из правой панели.\n"
                    "4. Нажмите «Настроить рабочее окружение», чтобы установить интеграцию и службу."
                ),
        ...
        def _apply_theme(self, style) -> None:
            ...
            style.configure("HeroTitle.TLabel", background=BASEALT_GRAPHITE, foreground=BASEALT_PAPER, font=(UI_FONT, 24, "bold"))
            style.configure("HeroSubtitle.TLabel", background=BASEALT_GRAPHITE, foreground="#E7DED6", font=(UI_FONT, 11))
            style.configure(
                "HeroBadge.TLabel",
            ...
            style.configure("SectionTitle.TLabel", background=BASEALT_PANEL, foreground=BASEALT_TEXT, font=(UI_FONT, 12, "bold"))
            style.configure("SectionTitleInverse.TLabel", background=BASEALT_GRAPHITE, foreground=BASEALT_PAPER, font=(UI_FONT, 12, "bold"))
            style.configure("Muted.TLabel", background=BASEALT_PANEL, foreground=BASEALT_MUTED, font=(UI_FONT, 10))
            style.configure("MutedInverse.TLabel", background=BASEALT_GRAPHITE, foreground="#D7CEC7", font=(UI_FONT, 10))
            style.configure("FieldLabel.TLabel", background=BASEALT_PANEL, foreground=BASEALT_MUTED, font=(UI_FONT, 9, "bold"))
            ...
            style.configure("TLabelframe.Label", background=BASEALT_PANEL, foreground=BASEALT_TEXT, font=(UI_FONT, 10, "bold"))
            style.configure(
                "TNotebook",
            ...
            style.configure("Primary.TButton", background=BASEALT_RED, foreground="#FFFFFF", borderwidth=0, padding=(14, 9), font=(UI_FONT, 10, "bold"))
            style.map("Primary.TButton", background=[("active", BASEALT_RED_DARK), ("disabled", "#C7B8AF")], foreground=[("disabled", "#F8F2ED")])
            style.configure("Secondary.TButton", background="#E8DDD4", foreground=BASEALT_TEXT, borderwidth=0, padding=(12, 9), font=(UI_FONT, 10, "bold"))
        ...
        def _open_auth_url(self) -> None:
            ...
            try:
                ...
                self._messagebox.showerror("CloudBridge", f"Не удалось открыть браузер:\n{error}")
        ...
        def _copy_auth_url(self) -> None:
            ...
            if copy_text_to_clipboard(url):
                self._status_var.set("Ссылка для входа скопирована")
        ...
        def _copy_auth_code(self) -> None:
            ...
            if copy_text_to_clipboard(code):
                self._status_var.set("Код авторизации скопирован")
        ...
        def _save_config(self, *, notify: bool = True) -> None:
            try:
                ...
                self._status_var.set(f"Настройки сохранены: {path}")
                self._log(f"Настройки сохранены: {path}")
                if notify:
                    self._messagebox.showinfo("CloudBridge", f"Настройки сохранены в файл:\n{path}")
            except Exception as error:
                self._messagebox.showerror("CloudBridge", str(error))
                self._log(f"Не удалось сохранить настройки: {error}")
        ...
        def _run_yandex_login(self) -> None:
            ...
            if not client_id or not client_secret:
                self._messagebox.showerror("CloudBridge", "Для входа в Яндекс нужны Client ID и Client secret.")
                return

            def worker() -> dict[str, str]:
                async def job() -> dict[str, str]:
                    def on_ready(prompt) -> None:
                        self._event_queue.put(("log", f"Ссылка для входа в Яндекс: {prompt.verification_url}"))
                        self._event_queue.put(("log", f"Код устройства Яндекса: {prompt.user_code}"))
            ...
            self._start_worker("Вход в Яндекс", worker, self._complete_yandex_login)
        ...
        def _complete_yandex_login(self, payload: dict[str, str]) -> None:
            ...
            self._messagebox.showinfo("CloudBridge", "Вход в Яндекс завершен.")

        def _run_nextcloud_login(self) -> None:
            server = self._nextcloud_url_var.get().strip()
            if not server:
                self._messagebox.showerror("CloudBridge", "Для входа в Nextcloud нужен адрес сервера.")
                return

            def worker() -> dict[str, str]:
                async def job() -> dict[str, str]:
                    def on_ready(prompt) -> None:
                        self._event_queue.put(("log", f"Ссылка для входа в Nextcloud: {prompt.login_url}"))
            ...
            self._start_worker("Вход в Nextcloud", worker, self._complete_nextcloud_login)
        ...
        def _complete_nextcloud_login(self, payload: dict[str, str]) -> None:
            ...
            self._save_config(notify=False)
            self._messagebox.showinfo("CloudBridge", "Вход в Nextcloud завершен.")
        ...
        def _run_desktop_setup(self) -> None:
            ...
            def worker() -> list[str]:
                ...
                messages = [f"База данных инициализирована: {config.database_path}"]
                ...
                if install_filemanager:
                    ...
                    if resolved_manager == "auto":
                        ...
                        if detected is None:
                            raise RuntimeError("Не удалось определить поддерживаемый файловый менеджер.")
                    ...
                    if resolved_manager == "nautilus":
                        ...
                        messages.append(f"Расширение Nautilus: {result.extension_path}")
                    elif resolved_manager == "thunar":
                        result = install_thunar_integration(config, repo_root=None, launcher_command=launcher_command)
                        messages.append(f"Конфиг Thunar UCA: {result.config_path}")
                    elif resolved_manager == "nemo":
                        result = install_nemo_integration(config, repo_root=None, launcher_command=launcher_command)
                        messages.append("Действия Nemo: " + ", ".join(str(path) for path in result.action_paths))
                    else:
                        result = install_caja_integration(config, repo_root=None, launcher_command=launcher_command)
                        messages.append(f"Расширение Caja: {result.extension_path}")
                        messages.append("Действия Caja: " + ", ".join(str(path) for path in result.action_paths))
                ...
                if install_service:
                    ...
                    messages.append(f"Файл службы systemd: {result.unit_path}")
                    if shutil.which("systemctl"):
                        subprocess.run(["systemctl", "--user", "daemon-reload"], check=False)
                        subprocess.run(["systemctl", "--user", "enable", "--now", f"{service_name}.service"], check=False)
                        messages.append(f"Служба systemd запрошена: {service_name}.service")

                return messages

            self._start_worker("Настройка рабочего окружения", worker, self._complete_desktop_setup)

        def _complete_desktop_setup(self, messages: list[str]) -> None:
            for message in messages:
                self._log(message)
            self._messagebox.showinfo("CloudBridge", "Настройка рабочего окружения завершена.")
        ...
        def _start_worker(
            ...
            if self._busy:
                self._messagebox.showwarning("CloudBridge", "Сейчас уже выполняется другая операция.")
                return
            self._set_busy(True)
            self._log(f"{label}: запуск")
        ...
        def _pump_events(self) -> None:
            while True:
                ...
                if event_type == "error":
                    ...
                    self._messagebox.showerror("CloudBridge", f"{label}: ошибка:\n{error}")
                    continue
                if event_type == "yandex-prompt":
                    verification_url, user_code = payload  # type: ignore[misc]
                    self._set_auth_prompt(
                        title="Вход в Яндекс по коду устройства",
                        url=verification_url,
                        code=user_code,
                        help_text="Откройте ссылку и введите код, который показан здесь. Яндекс не присылает этот код письмом или по SMS.",
                    )
                    self._status_var.set("Код для входа в Яндекс готов")
                    self._messagebox.showinfo(
                        "CloudBridge: вход в Яндекс",
                        "Откройте ссылку для подтверждения и введите код из правой панели.",
                    )
                    continue
                if event_type == "nextcloud-prompt":
                    login_url = payload  # type: ignore[assignment]
                    self._set_auth_prompt(
                        title="Вход в Nextcloud через браузер",
                        url=login_url,
                        code="",
                        help_text="Откройте ссылку и завершите вход в браузере. Отдельный код не нужен.",
                    )
                    self._status_var.set("Ссылка для входа в Nextcloud готова")
                    self._messagebox.showinfo(
                        "CloudBridge: вход в Nextcloud",
                        "Откройте ссылку из правой панели и завершите вход в браузере.",
                    )
                    continue
                if event_type == "success":
                    label, callback, result = payload  # type: ignore[misc]
                    self._set_busy(False)
                    self._log(f"{label}: завершено")
                    callback(result)
            self._root.after(200, self._pump_events)
        ...
        def _set_busy(self, busy: bool) -> None:
            ...
            self._status_var.set("Выполняется операция..." if busy else f"Конфиг: {self._base_config.resolved_config_path}")
