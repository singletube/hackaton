...
from ..core.xattr import get_placeholder_remote_path, remove_placeholder_remote_path
...
class SyncEventHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        if self._is_inside_sync_root(event.src_path):
            self.loop.call_soon_threadsafe(self.queue.put_nowait, ("created_in_sync", event.src_path, 0))
        else:
    ...
    def on_moved(self, event):
        ...
        if src_inside and dest_inside:
            ...
            self.loop.call_soon_threadsafe(self.queue.put_nowait, ("created_in_sync", event.dest_path, 0))
...
class AsyncWatcher:
    ...
    async def _resolve_placeholder_remote_path(self, local_path: str, expected_remote_path: str | None = None) -> str | None:
        xattr_remote = get_placeholder_remote_path(local_path)
        if xattr_remote:
            return xattr_remote
        ...
        if len(matches) > 1:
            logger.info(
                "Cannot infer copied placeholder source for %s: %d offline files have the same name",
                local_path,
                len(matches),
            )
        return None
    ...
    async def _hydrate_path(self, remote_path: str, dest_path: str, delete_remote: bool = False):
        if not os.path.exists(dest_path):
            logger.info("Destination disappeared before hydration: %s", dest_path)
            return
        if os.path.getsize(dest_path) != 0:
            logger.info("Destination already contains real data, leaving as-is: %s", dest_path)
            return

        logger.info("Hydrating %s from %s", dest_path, remote_path)
        self._notify_copy_download("CloudBridge", f"Downloading {os.path.basename(dest_path)}. Please wait.")
    ...
    async def _handle_created_in_sync(self, handler: SyncEventHandler, local_path: str, attempt: int):
        ...
        if is_ignored(remote_path):
            logger.info("Ignoring local-only path %s (created_in_sync)", remote_path)
            return
        ...
        existing_item = await self.manager.db.get_item(remote_path)
        if existing_item and existing_item["status"] == FileStatus.OFFLINE.value:
            logger.debug("Ignoring existing offline placeholder create for %s", local_path)
            return

        source_remote = await self._resolve_placeholder_remote_path(local_path)
        if not source_remote:
            if attempt < 3:
                asyncio.get_running_loop().call_later(
                    0.5,
                    self.queue.put_nowait,
                    ("created_in_sync", local_path, attempt + 1),
                )
            else:
                logger.warning("Safety: skipping unknown 0-byte file inside sync folder: %s", local_path)
            return

        logger.info("Sync-folder placeholder copy detected: %s -> %s", source_remote, remote_path)
        await self._hydrate_path(source_remote, local_path, delete_remote=False)
        await self.manager.upload_file(local_path, remote_path)
                    ...
                    if action == "upload":
                        ...
                        if os.path.exists(local_path):
                            ...
                            if is_stub:
                                ...
                                await self.manager.upload_file(local_path, remote_path)
                    elif action == "created_in_sync":
                        await self._handle_created_in_sync(handler, local_path, task[2])
                        ...
                        if pending:
                            await self._hydrate_path(pending["remote_path"], dest_path, delete_remote=True)
                            continue

                        source_remote = await self._resolve_placeholder_remote_path(dest_path)
                        if source_remote:
                            logger.info("External placeholder copy detected: %s -> %s", source_remote, dest_path)
                            await self._hydrate_path(source_remote, dest_path, delete_remote=False)
                            continue
                        ...
                        if attempt < 3:
                            loop.call_later(
                                0.5,
                                self.queue.put_nowait,
                                ("external_created", dest_path, attempt + 1),
                            )
                        else:
